Con
  ...
  PING_0        = 0
  PING_1        = 1
  ' I added two pins for ping sensors
  PING_2        = 2
  'PING_3        = 3
  ' Encoders
  ENCODERS_PIN  = 8
  ...
  PINGABLE      = %00000000_00000000_00001111_11111111
  INITIAL_GPIO  = |< SSR_A | |< SSR_B | |< SSR_C
  ' I added two pins for ping sensors
  ' INITIAL_PING  = |< PING_0 | |< PING_1| |< PING_2 | |<PING_3
  INITIAL_PING  = |< PING_0 | |< PING_1| |< PING_2
...
PrevMotPoss   long      0[HALF_SEC * 2]                 ' Previous motor possitions
...
PrvMotPossIdx byte      0                               ' Previous motor possitions index
...
KeepAlive     byte      True                            ' State of position control cog
...
Pri Parse | Index, Parameter[3]                         '' Parse the command in the input buffer
          ...
          abort @InvalidParameter
{
    ...
    elseif strcomp(@InputBuffer, string("VERB"))        ' Command: Set verbose mode
      ...
      case Parameter
        ...
        other:
          abort @InvalidParameter
    elseif strcomp(@InputBuffer, string("WATCH"))       ' Command: Set watch mode
      Parameter := ParseHex(NextParameter)
      CheckLastParameter
      case Parameter
        0..1:
    ...
    elseif strcomp(@InputBuffer, string("PWR"))        ' TODO: remove
      CheckLastParameter
      OutputHex(MotorPower[LEFT_MOTOR], 8)
      OutputChr(" ")
      OutputHex(MotorPower[RIGHT_MOTOR], 8)
    ...
    elseif strcomp(@InputBuffer, string("SETV"))        ' TODO: remove
      CheckLastParameter
      OutputHex(SetVelocity[LEFT_MOTOR], 8)
      OutputChr(" ")
      OutputHex(SetVelocity[RIGHT_MOTOR], 8)
    else
...
Pri PDLoop | NextCNT                                    '' Measure, set, and maintain wheel position

  
  Encoders.Start(ENCODERS_PIN, 2, 2, @MotorPosition)                                
  ...
  repeat
    waitcnt(NextCNT += constant(_clkfreq / DIVISOR / 2))' Loop DIVSOR times per second (first half)
    Motors.Right(PDIteration(RIGHT_MOTOR))              ' Service right wheel and motor
    waitcnt(NextCNT += constant(_clkfreq / DIVISOR / 2))' Loop DIVSOR times per second (second half)
...
'' Read the wheel's speed and position, and set its power
  ...
  ' Compare current position to position from 1/2 second ago to determine speed in positions per second
  MotorSpeed[Side] := MotorPositionSample - PrevMotPoss[Side * HALF_SEC + PrvMotPossIdx]
  PrevMotPoss[Side * HALF_SEC + PrvMotPossIdx++] := MotorPositionSample ' Record current position for next round
  PrvMotPossIdx //= HALF_SEC

  if ||MotorSpeed[Side] =< 1                            ' Keep track of how long the motor hasn't been moving
    StillCnt[Side]++
    StillCnt[Side] <#= DIVISOR
  else
    StillCnt[Side]~
  ...
  case Mode                                             ' Set motor power based on current control method
    ...
    VELOCITY:                                           ' Maintain the motors at a set velocity
      ' MidVelocity / HALF_SEC approaches SetVelocity as limited by MaxPosAccel
      MidVelAcc[Side] += -MaxPosAccel #> (SetVelocity[Side] - MidVelocity[Side]) * HALF_SEC - MidVelAcc[Side] <# MaxPosAccel
...
'   ARC_POSITION:                                       ' Turn along an arc
    ...
    POSITION:                                           ' Travel to a set position
      ' MidVelocity / HALF_SEC approaches SetVelocity as limited by MaxPosAccel / DIVISOR
      MidVelAcc[Side] += -MaxPosAccel #> (SetVelocity[Side] - MidVelocity[Side]) * HALF_SEC - MidVelAcc[Side] <# MaxPosAccel
      MidVelocity[Side] += MidVelAcc[Side] / HALF_SEC
      MidVelAcc[Side] //= HALF_SEC

      ' MidPosition approaches SetPosition as limited by the deceleration curve
      Limit := MidVelocity[Side] * HALF_SEC + MidVelAcc[Side] <# ^^(constant(8 * HALF_SEC * HALF_SEC) * (SetPosition[Side] - MidPosition[Side])/ MaxPosAccel) * MaxPosAccel / 2
      MidPosAcc[Side] += -Limit #> (SetPosition[Side] - MidPosition[Side]) * constant(HALF_SEC * HALF_SEC) + MidPosAcc[Side] <# Limit
      MidPosition[Side] += MidPosAcc[Side] / constant(HALF_SEC * HALF_SEC)
      MidPosAcc[Side] //= constant(HALF_SEC * HALF_SEC)
...
Pri ChecksumParse | Index, Checksum, RemoteChecksum '' Verify checksum and parse input command
...
Pri SendChecksumResponse | Index, Checksum              '' Transmit the string in the output buffer, and its checksum, then clear the buffer

  Checksum := PROMPT_SUM                                ' Generate the checksum
  repeat Index from 0 to strsize(@OutputBuffer)
    Checksum += OutputBuffer[Index]
  ...
  Term.Tx(Checksum >> 8)                                ' Transmit the Checksum
  Term.Tx(Checksum)
