
(cl:in-package :asdf)

(defsystem "morelab_robot_platform-msg"
  :depends-on (:roslisp-msg-protocol :roslisp-utils )
  :components ((:file "_package")
    (:file "Voltages" :depends-on ("_package_Voltages"))
    (:file "_package_Voltages" :depends-on ("_package"))
    (:file "BatteryLevel" :depends-on ("_package_BatteryLevel"))
    (:file "_package_BatteryLevel" :depends-on ("_package"))
    (:file "Speech" :depends-on ("_package_Speech"))
    (:file "_package_Speech" :depends-on ("_package"))
    (:file "DistanceWheel" :depends-on ("_package_DistanceWheel"))
    (:file "_package_DistanceWheel" :depends-on ("_package"))
    (:file "Velocity" :depends-on ("_package_Velocity"))
    (:file "_package_Velocity" :depends-on ("_package"))
    (:file "StopSlidingSignal" :depends-on ("_package_StopSlidingSignal"))
    (:file "_package_StopSlidingSignal" :depends-on ("_package"))
    (:file "ADC" :depends-on ("_package_ADC"))
    (:file "_package_ADC" :depends-on ("_package"))
    (:file "Distances" :depends-on ("_package_Distances"))
    (:file "_package_Distances" :depends-on ("_package"))
    (:file "KeyStroke" :depends-on ("_package_KeyStroke"))
    (:file "_package_KeyStroke" :depends-on ("_package"))
    (:file "Ping" :depends-on ("_package_Ping"))
    (:file "_package_Ping" :depends-on ("_package"))
    (:file "SpeedWheel" :depends-on ("_package_SpeedWheel"))
    (:file "_package_SpeedWheel" :depends-on ("_package"))
  ))