...
public:
   ...
   bool driveForwardOdom(double distance, double speed)
   {
    ...
    while (!done && nh_.ok())
    {
      ...
      ROS_INFO("Traveled Distance = %f (m)", dist_moved);
  ...
  // Turn
  bool driveTurnOdom(bool clockwise, double radians, double angular_speed)
  {
     ...
     while (!done && nh_.ok())
     {
       ...
       ROS_INFO("Turned Angle = %f (rad)", angle_turned);
...
{
  //init the ROS node
  ros::init(argc, argv, "eddie_controller_command");
  ros::NodeHandle nh;
  RobotDriver driver(nh);
  driver.driveForwardOdom(1.0, 1);
  //driver.driveTurnOdom(false, -0.785, 1);
}
